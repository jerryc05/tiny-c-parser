...
let rec string_of_stmt (s : stmt) : string = match s with
  ...
  | FunctionDecl(str, data_type, paramlist, stmt) -> "FunctionDecl(\"" ^ str ^ "\", " ^ string_of_data_type data_type ^ ", " ^ string_of_list (fun (s, dt) -> "(\"" ^ s ^ "\", " ^ string_of_data_type dt ^ ")" ) paramlist ^ ", " ^ string_of_stmt stmt ^ ")"
  | Return(e) -> "Return(" ^ string_of_expr e ^ ")"
  | _ -> ""

and string_of_list ?newline:(newline=false) (f : 'a -> string) (l : 'a list) : string =
